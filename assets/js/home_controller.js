            ...
            $scope.footnotes = Footnotes.query({
                ...
                if (!document.getElementById("fn_" + translation_id)) {
                    ...
                    for (index = 0; index < dataLength; ++index) {
                        ...
                        //dataContent = data[index].replace(/(\d{1,3}:\d{1,3})/g, "<a href='javascript: angular.element(document.getElementById(\"MainCtrl\")).scope().showVerseDetail(\"$1\")' data-target='#detailedVerseModal' data-dismiss='modal' data-toggle='modal'>$1</a>");
                        dataContent = data[index].replace(/(\d{1,3}):(\d{1,3})/g, "<a ng-href='#MainCtrl' onclick='javascript: angular.element(document.getElementById(\"MainCtrl\")).scope().showVerseDetail($1*1000+$2)' data-target='#detailedVerseModal' data-dismiss='modal' data-toggle='modal'>$1</a>");
